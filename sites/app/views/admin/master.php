...
$cats = $db->query("SELECT * FROM admin_categories WHERE parent_id IS NULL ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);

foreach ($cats as $cat): ?>
    <li class="nav-item">
        <a href="/admin/<?php echo $cat['slug']; ?>" class="nav-link nav-toggle">
            <i class="fa fa-folder"></i>
            <span class="title"><?php echo htmlspecialchars($cat['name']); ?></span>
            <span class="arrow"></span>
        </a>
        <?php
        $subcats = $db->prepare("SELECT * FROM admin_categories WHERE parent_id = ?");
        $subcats->execute([$cat['id']]);
        $subs = $subcats->fetchAll(PDO::FETCH_ASSOC);

        if ($subs): ?>
            <ul class="sub-menu">
                <?php foreach ($subs as $sub): ?>
                    <li class="nav-item">
                        <a href="/admin/<?php echo $sub['slug']; ?>" class="nav-link">
