...
<body>
    ...
    <section class="trust-badges">
        <div class="container">
            ...
            <div class="badges-grid">
                ...
                <?php endforeach; else: ?>
                    <div class="badge-card"><div class="badge-icon">⚙️</div><h3>İleri Teknoloji</h3><p>Son model CNC torna ve freze makineleri ile hassas üretim</p></div>
                    <div class="badge-card"><div class="badge-icon">🔧</div><h3>Deneyimli Ekip</h3><p>20 yıllık tecrübeli ustalar ve teknik ekip</p></div>
                    <div class="badge-card"><div class="badge-icon">⚡</div><h3>Hızlı Teslimat</h3><p>Acil işleriniz için aynı gün teslimat imkanı</p></div>
                    <div class="badge-card"><div class="badge-icon">✓</div><h3>Kalite Garantisi</h3><p>Ölçü ve dayanıklılık garantili üretim</p></div>
                <?php endif; ?>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            ...
            <div class="vehicles-grid">
                <?php if(!empty($latestProducts)): foreach($latestProducts as $product): ?>
                    <div class="vehicle-card" onclick="trackView(<?php echo $product['id']; ?>)">
                        <div class="product-image-slider">
                                    ...
                                    <div class="swiper-wrapper">
                                        <?php foreach($product['images'] as $img): ?>
                                            <div class="swiper-slide">
                                                <img src="<?php echo htmlspecialchars($img['image_url']); ?>" alt="<?php echo htmlspecialchars($product['title']); ?>">
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                    <?php if(count($product['images'])>1): ?>
                            ...
                            <?php else: ?>
                                <img src="<?php echo htmlspecialchars($product['image']); ?>" alt="<?php echo htmlspecialchars($product['title']); ?>" style="width:100%;height:200px;object-fit:cover">
                            <?php endif; ?>
    ...
    <section class="section services" id="hizmetler">
        <div class="container">
            ...
            <div class="services-grid">
                ...
                <?php endforeach; else: ?>
                    <div class="service-card"><h3>⚙️ CNC Torna İşleme</h3><p>Hassas ölçülerde torna, yüzey tornalama ve diş açma işlemleri</p></div>
                    <div class="service-card"><h3>🔧 CNC Freze</h3><p>Karmaşık geometrilerde frezeleme ve delme operasyonları</p></div>
                    <div class="service-card"><h3>🛠️ Makina Bakım-Onarım</h3><p>Tüm sanayi makinelerinin periyodik bakım ve arıza onarımı</p></div>
                    <div class="service-card"><h3>📐 Yedek Parça İmalatı</h3><p>Teknik resme göre özel yedek parça üretimi</p></div>
                    <div class="service-card"><h3>⚡ Kaynak İşleri</h3><p>TIG, MIG/MAG ve elektrod kaynağı ile onarım ve imalat</p></div>
                    <div class="service-card"><h3>🔩 Montaj Hizmeti</h3><p>Makina ve ekipman montaj, revizyon ve iyileştirme çalışmaları</p></div>
                <?php endif; ?>
            </div>
        </div>
    </section>
    ...
    <section class="section testimonials" id="referanslar">
        <div class="container">
            ...
            <?php endforeach; else: ?>
                <div class="testimonial-card"><p>"Acil bir parça ihtiyacımız olduğunda aynı gün içinde üretip teslim ettiler. Kaliteli işçilik ve güvenilir hizmet."</p><div class="testimonial-author">- Ahmet Demir, Üretim Müdürü - ABC Tekstil</div></div>
                <div class="testimonial-card"><p>"20 yıldır makina bakımlarımızı yaptırıyoruz. Profesyonel ekip ve uygun fiyatlar."</p><div class="testimonial-author">- Fatma Yıldız, İşletme Sahibi - Yıldız Gıda</div></div>
                <div class="testimonial-card"><p>"CNC torna işlemlerinde hassasiyet ve zamanında teslimat konusunda çok memnunuz."</p><div class="testimonial-author">- Mehmet Kaya, Teknik Şef - DEF Otomotiv</div></div>
            <?php endif; ?>
        </div>
    </section>
        ...
        <div class="container">
            ...
            <div class="contact-grid">
                <div class="contact-info">
                    ...
                    <div style="margin-top:2rem"><a href="https://wa.me/<?php echo htmlspecialchars($settings['whatsapp_number']); ?>" class="whatsapp-btn" target="_blank">💬 WhatsApp ile İletişime Geç</a></div>
                </div>
                <div class="contact-info">
                    ...
                    <div style="margin-top:2rem">
                        ...
                        <div style="display:flex;gap:1rem;margin-top:1rem">
                            <a href="<?php echo htmlspecialchars($settings['facebook_url']); ?>" style="color:#2c3e50;font-size:2rem">📘</a>
                            <a href="<?php echo htmlspecialchars($settings['instagram_url']); ?>" style="color:#2c3e50;font-size:2rem">📸</a>
                            <a href="<?php echo htmlspecialchars($settings['youtube_url']); ?>" style="color:#2c3e50;font-size:2rem">🎥</a>
                        </div>
                    </div>
                </div>
                <div class="contact-info">
    ...
    <script>
        ...
        <?php foreach($latestProducts as $product): ?>
            <?php if(!empty($product['images']) && count($product['images'])>1): ?>
                new Swiper('.product-swiper-<?php echo $product['id']; ?>',{
                    ...
                    speed:800,
                    pagination:{el:'.product-swiper-<?php echo $product['id']; ?> .swiper-pagination',clickable:true},
                    navigation:{nextEl:'.product-swiper-<?php echo $product['id']; ?> .swiper-button-next',prevEl:'.product-swiper-<?php echo $product['id']; ?> .swiper-button-prev'}
                });
            <?php endif; ?>
        <?php endforeach; ?>
        
        function trackView(productId){fetch('?view_product='+productId)}
        let selectedOfferType='',currentProductId=null,currentProductTitle='';
        function openOfferModal(productId=null,productTitle=''){currentProductId=productId;currentProductTitle=productTitle;document.getElementById('offerModal').style.display='block';document.body.style.overflow='hidden'}
        ...
        function selectOfferType(type){selectedOfferType=type;document.getElementById('offerType').value=type;document.querySelectorAll('.offer-type-card').forEach(card=>card.classList.remove('active'));event.target.closest('.offer-type-card').classList.add('active');document.getElementById('projectInfoGroup').style.display='block'}
